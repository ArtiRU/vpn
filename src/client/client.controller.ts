...
export class ClientController {
    ...
    @ApiResponse({ status: 200, description: 'List of available servers', type: [ServerInfoDto] })
    ...
    @ApiParam({ name: 'countryCode', description: 'Two-letter country code (e.g., US, DE, UK)', example: 'DE' })
    @ApiResponse({ status: 200, description: 'List of servers in specified country', type: [ServerInfoDto] })
    @ApiResponse({ status: 401, description: 'Unauthorized' })
    async getServersByCountry(@Param('countryCode') countryCode: string): Promise<ServerInfoDto[]> {
        return this.clientService.getServersByCountry(countryCode);
    }

    @Get('subscription/status')
    @ApiOperation({ summary: 'Get current user subscription status' })
    @ApiResponse({ status: 200, description: 'Subscription status', type: SubscriptionStatusDto })
    @ApiResponse({ status: 401, description: 'Unauthorized' })
    async getSubscriptionStatus(@GetUser() user: User): Promise<SubscriptionStatusDto> {
        return this.clientService.getSubscriptionStatus(user.id);
    }

    @Get('config')
    @ApiOperation({ summary: 'Get VPN configuration for specific server' })
    @ApiQuery({ name: 'server_id', description: 'Server ID to get configuration for', example: 1, type: Number })
    @ApiResponse({ status: 200, description: 'VPN configuration', type: VpnConfigDto })
    @ApiResponse({ status: 401, description: 'Unauthorized' })
    @ApiResponse({ status: 403, description: 'No active subscription or config expired' })
    @ApiResponse({ status: 404, description: 'Configuration not found for this server' })
    async getVpnConfig(
        @GetUser() user: User,
    ...
    @ApiResponse({ status: 200, description: 'List of user VPN configurations', type: [VpnConfigDto] })
    @ApiResponse({ status: 401, description: 'Unauthorized' })
